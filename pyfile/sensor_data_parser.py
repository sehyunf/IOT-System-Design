...
def print_data(feature, feature_unit, sensor_data):
    try:
        result = " | ".join(
                f"{f}: {d} {u}"
                for f, u, d in zip(feature, feature_unit, sensor_data.values())
            )
        return ('[Received Data] ' + result)
    except Exception as e:                    # 전체 처리 과정에서 예외 발생 시
        print(f"데이터 출력 오류: {e}")      # 오류 메시지 출력
        return None
...
if sensor_sock:                                                # 블루투스 소켓이 연결되어 있다면
    try:
        while True:                                            # 무한 루프
            ...
            if sensor_data:
                result = print_data(feature, feature_unit, sensor_data)
                if result:
                    print(result)
